...
class MySQL 
{
	...
	{
		$this->conn = mysql_connect($GLOBALS['mysql_host'], $GLOBALS['mysql_username'], $GLOBALS['mysql_password']);
		if(!$this->conn)
			die('Could not connect, check config.php for settings: ' . mysql_error());
		
		mysql_select_db($GLOBALS['mysql_dbname']) or die("couldn't select database ".$GLOBALS['mysql_dbname'].". Check config.php for settings. MySQL error: ".mysql_error()."");
	}
